...
if __name__ == '__main__':
    ...
    filenames = {
        'origin': '../../dataset/letter/data_origin_epsilon',
        'error': '../../dataset/letter/data_error_epsilon'
    }
    ...
    for epsilon in [3, 4, 5, 6, 7, 8, 9, 10]:
        print epsilon
        rms_lists = dict()
            ...
            for method_name, exp_func in exp_methods:
                ...
                if exp_func is not None:
                    rms, precision, recall, accuracy, repair_distance, data_size = exp_func(
                        schema, epsilon=epsilon, outliers=None, filenames=filenames,
                        neighbor_k=4, used_attrs=used_attrs)
                else:
        ...
        output_content_rms += str(epsilon) + '\t%f\t%f\t%f\t%f\r\n' % tuple(rms)
        output_content_accuracy += str(epsilon) + '\t%f\t%f\t%f\t%f\r\n' % tuple(accuracy)
        output_content_distance += str(epsilon) + '\t%f\t%f\t%f\t%f\r\n' % tuple(repair_distance)
        output_content_time += str(epsilon) + '\t%f\t%f\t%f\t%f\r\n' % tuple(time_used)
    ...
    with open('result/epsilon_rms_wisconsin.dat', 'w') as f:
        f.write(output_content_rms)
    with open('result/epsilon_accuracy_wisconsin.dat', 'w') as f:
        f.write(output_content_accuracy)
    with open('result/epsilon_repair_distance_wisconsin.dat', 'w') as f:
        f.write(output_content_distance)
    with open('result/epsilon_time_wisconsin.dat', 'w') as f:
