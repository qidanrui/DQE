...
if __name__ == '__main__':
    ...
    for size in [1000, 2000, 3000, 4000, 5000]:
        ...
        for seed in random_seeds:
            ...
            set_error_config('data_size', size)
            outliers = begin_add_errors(origin_schema, '../../dataset/letter/data', filenames, classes=30,
                                        used_attrs=used_attrs, modifiable_attrs=modifiable_attrs)
            ...
            for method_name, exp_func in exp_methods:
                ...
                if exp_func is not None:
                    rms, precision, recall, accuracy, repair_distance, data_size = exp_func(
                        origin_schema, filenames=filenames, outliers=outliers,
        ...
        output_content_rms += str(size) + '\t%f\t%f\t%f\t%f\r\n' % tuple(rms)
        output_content_accuracy += str(size) + '\t%f\t%f\t%f\t%f\r\n' % tuple(accuracy)
        output_content_distance += str(size) + '\t%f\t%f\t%f\t%f\r\n' % tuple(repair_distance)
        output_content_time += str(size) + '\t%f\t%f\t%f\t%f\r\n' % tuple(time_used)
    ...
    with open('result/size_rms_wisconsin.dat', 'w') as f:
        f.write(output_content_rms)
    with open('result/size_accuracy_wisconsin.dat', 'w') as f:
        f.write(output_content_accuracy)
    with open('result/size_repair_distance_wisconsin.dat', 'w') as f:
        f.write(output_content_distance)
    with open('result/size_time_wisconsin.dat', 'w') as f:
