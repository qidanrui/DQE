...
if __name__ == '__main__':
    ...
    filenames = {
        'origin': '../../dataset/letter/data_origin_attrs',
        'error': '../../dataset/letter/data_error_attrs'
    }
    ...
    for attrs_num in [8, 9, 10, 11, 12, 13, 14]:
        print attrs_num
        used_attrs = ['attr%d' % i for i in xrange(1, attrs_num + 1)]
        ...
        for seed in random_seeds:
            ...
            outliers = begin_add_errors(origin_schema, origin_filename, filenames, classes=100,
                                        modifiable_attrs=modifiable_attrs, used_attrs=used_attrs)
            ...
            for method_name, exp_func in exp_methods:
                ...
                if exp_func is not None:
                    rms, precision, recall, accuracy, repair_distance, data_size = exp_func(
                        schema, outliers=outliers, filenames=filenames,
                        neighbor_k=5, used_attrs=used_attrs)
                else:
        ...
        output_content_rms += str(attrs_num) + '\t%f\t%f\t%f\t%f\r\n' % tuple(rms)
        output_content_accuracy += str(attrs_num) + '\t%f\t%f\t%f\t%f\r\n' % tuple(accuracy)
        output_content_distance += str(attrs_num) + '\t%f\t%f\t%f\t%f\r\n' % tuple(repair_distance)
        output_content_time += str(attrs_num) + '\t%f\t%f\t%f\t%f\r\n' % tuple(time_used)
    ...
    with open('result/attrs_rms_wisconsin.dat', 'w') as f:
        f.write(output_content_rms)
    with open('result/attrs_accuracy_wisconsin.dat', 'w') as f:
        f.write(output_content_accuracy)
    with open('result/attrs_repair_distance_wisconsin.dat', 'w') as f:
        f.write(output_content_distance)
    with open('result/attrs_time_wisconsin.dat', 'w') as f:
